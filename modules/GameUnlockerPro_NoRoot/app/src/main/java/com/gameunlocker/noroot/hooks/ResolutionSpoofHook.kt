 ...
 * 分辨率&画质伪装Hook
 *
 * 功能：低分辨率手机伪装2K屏幕，强制游戏加载高清材质包
 *
 * 硬性限制：
 *  - 仅修改应用读取到的Display/DisplayMetrics值
 *  - 实际GPU渲染分辨率由游戏渲染引擎决定，本Hook不改变硬件分辨率
 *  - 在某些游戏上可能因分辨率不匹配导致UI错位
 */
object ResolutionSpoofHook {
    ...
    private fun hookDisplaySize(lpparam: XC_LoadPackage.LoadPackageParam, w: Int, h: Int) {
        try {
            ...
            val pt = XposedHelpers.findClass("android.graphics.Point", lpparam.classLoader)
            ...
            LogX.d("Display分辨率伪装完成")
        } catch (e: Exception) {
            LogX.e("Display分辨率Hook异常", e)
        }
    }

    private fun hookDisplayMetrics(lpparam: XC_LoadPackage.LoadPackageParam, w: Int, h: Int, d: Int) {
        try {
            val dmc = XposedHelpers.findClass("android.util.DisplayMetrics", lpparam.classLoader)
            ...
            LogX.d("DisplayMetrics伪装完成")
        } catch (e: Exception) {
            LogX.e("DisplayMetrics Hook异常", e)
        }
    }
}
