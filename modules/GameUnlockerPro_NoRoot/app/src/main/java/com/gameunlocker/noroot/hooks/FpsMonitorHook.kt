...
/**
 * FPS Monitor Overlay（实验性，NoRoot 版）
 *
...
object FpsMonitorHook {
    ...
    fun apply(lpparam: XC_LoadPackage.LoadPackageParam, cfg: GameConfig) {
        LogX.i("【实验性】FPS Monitor Overlay 启动（NoRoot）")
                ...
                object : XC_MethodHook() {
                    override fun beforeHookedMethod(p: MethodHookParam) {
                        try {
                            ...
                            LogX.e("Choreographer doFrame(NoRoot) 异常", e)
                        }
                    }
                })
            LogX.hookSuccess("Choreographer", "doFrame->FPS(NoRoot)")
        } catch (e: Exception) {
            LogX.e("Hook Choreographer doFrame(NoRoot) 异常", e)
        }
    }
            ...
            XposedHelpers.findAndHookMethod(
                ...
                object : XC_MethodHook() {
                    override fun afterHookedMethod(p: MethodHookParam) {
                        try {
                            LogX.d("FPS(NoRoot): ${String.format("%.1f", currentFps)} fps | jank=$jankCount")
                        } catch (e: Exception) {
                            LogX.e("addView FPS log(NoRoot) 异常", e)
                        }
                    }
                })
            LogX.hookSuccess("WindowManagerGlobal", "addView->FPSOverlay(NoRoot)")
        } catch (e: Exception) {
            LogX.e("Hook WindowManager.addView(NoRoot) 异常", e)
        }
    }
}
