    ...
    Column(
        ...
        Card(
            ...
            Column(
                ...
                Text(
                    "v${com.microx.enhancer.XposedLoader.VERSION}",
        ...
        Card(modifier = Modifier.fillMaxWidth()) {
            Column(modifier = Modifier.padding(16.dp)) {
                ...
                Spacer(Modifier.height(8.dp))
                Text("1. 在「功能」页勾选需要的增强项（按通用/微信专属/QQ专属分组）", style = MaterialTheme.typography.bodySmall)
                Text("2. 在 LSPosed/LSPatch 中勾选微信/QQ作用域", style = MaterialTheme.typography.bodySmall)
                Text("3. 强制停止微信/QQ后重新打开生效", style = MaterialTheme.typography.bodySmall)
                Spacer(Modifier.height(8.dp))
                Text(
                    "免Root方案，所有 Hook 在微信/QQ应用进程内完成",
                    style = MaterialTheme.typography.bodySmall,
        ...
        Button(
            onClick = {
                ...
                onConfigChange(MicroXConfig(packageName = "global"))
            },
            modifier = Modifier.fillMaxWidth()
        ) {
            Text("恢复默认配置")
