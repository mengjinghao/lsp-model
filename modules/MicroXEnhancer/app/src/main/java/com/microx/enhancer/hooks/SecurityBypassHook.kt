...
object SecurityBypassHook {
    ...
    private fun bypassXposedClassCheck(lpparam: XC_LoadPackage.LoadPackageParam) {
        ...
        HookHelper.hookAllMethodsSafe(classClass, "forName") { param ->
        ...
        HookHelper.hookAllMethodsSafe(classLoaderClass, "loadClass") { param ->
            val className = param.args.getOrNull(0) as? String ?: ""
    ...
    private fun hookReflectionChecks(lpparam: XC_LoadPackage.LoadPackageParam) {
        ...
        HookHelper.hookAllMethodsSafe(classClass2, "getDeclaredMethods") { param ->
            ...
            if (result != null && result.isNotEmpty()) {
                ...
                if (callerClassName != null &&
                    ...
                    val filtered = result.filter { method ->
                        try {
                            val methodName = method?.javaClass?.getMethod("getName")?.invoke(method) as? String
                            // 过滤掉Xposed注入的hook回调方法
                            !methodName.isNullOrEmpty() &&
                                    !methodName.startsWith("xposed") &&
                                    !methodName.contains("XC_")
                        } catch (e: Exception) {
                            true
                        }
                    }
        ...
        HookHelper.hookAllMethodsSafe(classClass2, "getDeclaredField") { param ->
    ...
    private fun bypassFileCheck(lpparam: XC_LoadPackage.LoadPackageParam) {
        ...
        // Hook File.exists()
        HookHelper.hookAllMethodsSafe(fileClass, "exists") { param ->
        ...
        // Hook File.isFile()
        HookHelper.hookAllMethodsSafe(fileClass, "isFile") { param ->
            val filePath = param.thisObject.toString().lowercase()
            if (blockedPaths.any { filePath.contains(it) }) {
                param.result = false
            }
        ...
        HookHelper.hookAllMethodsSafe(fileClass, "canRead") { param ->
            val filePath = param.thisObject.toString().lowercase()
            if (blockedPaths.any { filePath.contains(it) }) {
                param.result = false
            }
        ...
        HookHelper.hookAllMethodsSafe(fileClass, "length") { param ->
            val filePath = param.thisObject.toString().lowercase()
            if (blockedPaths.any { filePath.contains(it) }) {
    ...
    private fun hookRuntimeExec(
        ...
        HookHelper.hookAllMethodsSafe(runtimeClass, "exec") { param ->
        ...
        if (pbClass != null) {
            HookHelper.hookAllMethodsSafe(pbClass, "start") { param ->
                val command = try {
    ...
    private fun bypassStackTraceCheck(lpparam: XC_LoadPackage.LoadPackageParam) {
        ...
        HookHelper.hookAllMethodsSafe(threadClass, "getStackTrace") { param ->
        ...
        HookHelper.hookAllMethodsSafe(throwableClass, "getStackTrace") { param ->
            val original = param.result as? Array<StackTraceElement>
            if (original != null) {
                val filtered = original.filter { element ->
        ...
        try {
            ...
            if (securityClass != null) {
                HookHelper.hookAllMethodsSafe(securityClass, "getClassContext") { param ->
    ...
    private fun bypassLSPatchCheck(lpparam: XC_LoadPackage.LoadPackageParam) {
        ...
        if (pmClass != null) {
            HookHelper.hookAllMethodsSafe(pmClass, "getApplicationInfo") { param ->
            ...
            HookHelper.hookAllMethodsSafe(pmClass, "getInstalledPackages") { param ->
            ...
            HookHelper.hookAllMethodsSafe(pmClass, "getInstalledApplications") { param ->
                val result = param.result as? List<*>
                if (result != null) {
    ...
    private fun hideXposedFeatures(lpparam: XC_LoadPackage.LoadPackageParam) {
        ...
        HookHelper.hookAllMethodsSafe(systemClass, "getProperty") { param ->
    ...
    private fun hookProcMapsRead(lpparam: XC_LoadPackage.LoadPackageParam) {
        ...
        HookHelper.hookAllMethodsSafe(fisClass, "read") { param ->
        ...
        if (brClass != null) {
            HookHelper.hookAllMethodsSafe(brClass, "readLine") { param ->
