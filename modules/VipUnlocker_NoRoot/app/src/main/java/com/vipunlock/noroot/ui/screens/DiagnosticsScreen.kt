...
fun DiagnosticsScreen() {
    ...
    Column(
        ...
        DiagCard(
            ...
            detail = if (shizukuActive) "Shizuku 服务运行中（NoRoot 版可选，应用层 Hook 不依赖）"
                     else "Shizuku 未运行。NoRoot 版应用层 Hook 不依赖 Shizuku，可正常工作",
            ok = shizukuActive
        )
        Spacer(Modifier.height(8.dp))
        ...
        Card(modifier = Modifier.fillMaxWidth(),
            ...
            Column(modifier = Modifier.padding(16.dp)) {
                ...
                Text("• NoRoot 版应用层 Hook 不依赖 Shizuku", style = MaterialTheme.typography.bodySmall)
            }
        }
